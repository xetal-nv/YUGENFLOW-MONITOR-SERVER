...
func (jse *JsonSeriesEntries) ExpandEntries(ss SerieEntries) {
	// fmt.Println(ss)
	if ss.Stag != "" {
		...
		for j, i := range SpaceInfo[name] {
			// if i < len(ss.Sval) {
			jse.Sval = append(jse.Sval, JsonSingleEntry{Id: i, In: ss.Sval[j][0], Out: ss.Sval[j][1], Netflow: ss.Sval[j][0] + ss.Sval[j][1]})
			// }
		}
		//for _ = range SpaceInfo[name] {
		//	jse.Sval = append(jse.Sval, []int{0, 0})
		//}
	}
	return
}
